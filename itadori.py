...
# 実際の画像ファイルがある場合は、下のコメントを外してファイル名を合わせてください
# try:
#     set_bg_image("itadori.jpg") 
# except:
#     st.info("背景画像(itadori.jpg)を読み込めませんでした。")
        ...
        for p in sorted_parts:
            if not pack(p):
                sheets.append({'id': len(sheets)+1, 'used_h': p['d'] + self.kerf, 
                               'rows': [{'y': 0, 'h': p['d'], 'used_w': p['l'] + self.kerf if 'l' in p else p['w'] + self.kerf, 
...
if st.button("🧮 木取り図を作成する"):
    ...
    if not all_parts:
        ...
        st.divider()
        st.markdown('<button onclick="window.print()" style="padding: 10px 20px; background-color: #4CAF50; color: white; border: none; border-radius: 5px; cursor: pointer; width: 100%;">🖨️ 木取り図を印刷する</button>', unsafe_allow_html=True)
